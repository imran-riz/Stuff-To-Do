    ...
    public void setCompleted(Boolean completed) {
        ...
        if (this.repeat != Repeat.NONE) {
            if (this.repeat == Repeat.DAILY) {
                ...
                this.reminderDate = this.reminderDate.plusWeeks(1);
            }
             else if (this.repeat == Repeat.MONTHLY) {
                this.dueDate = this.dueDate.plusMonths(1);
                this.reminderDate = this.reminderDate.plusMonths(1);
            }
        }
    }
    ...
    private void scheduleReminderAlert() {
        if (this.reminderDate.equals(LocalDate.now())) {
            if (LocalTime.now().isBefore(this.reminderTime)) {
                System.out.println("Setting reminder alert for: " + this.getText());
